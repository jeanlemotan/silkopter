...
public:
    ...
    size_t get_inner_qualified_type_count() const override;
    std::shared_ptr<const Qualified_Type> get_inner_qualified_type(size_t idx) const override;
    boost::optional<size_t> find_inner_qualified_type_idx(std::shared_ptr<const IType> type) const override;

protected:
    Result<void> validate_attribute(IAttribute const& attribute) override;

private:
    std::vector<std::shared_ptr<const Qualified_Type>> m_inner_types;
