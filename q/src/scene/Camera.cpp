...
{
	auto mat = im;
	auto x = im.get_axis_x();
	auto y = im.get_axis_y();
	auto z = im.get_axis_z();
	mat.set_axis_x(x);
	mat.set_axis_y(z);
	mat.set_axis_z(-y);
	return mat;
...
math::line3f Camera::get_ray_from_screen_coordinates(math::vec2f const& pos) const
{
	...
	auto far_left_up = f.get_far_left_up();
	left_to_right = f.get_far_right_up()  - far_left_up;
	up_to_down    = f.get_far_left_down() - far_left_up;

	ln.end = far_left_up + (left_to_right * dx) + (up_to_down * dy);
	return ln;
}
...
{
	m_front_vector = math::normalized(math::rotate(get_final_rotation(), scene::get_front_vector()));
	m_up_vector = math::normalized(math::rotate(get_final_rotation(), scene::get_up_vector()));
	m_right_vector = math::normalized(math::rotate(get_final_rotation(), scene::get_right_vector()));
}
...
void Camera::set_coordinate_system(math::quatf const& rot)
{
	m_view_matrix_dirty = m_view_projection_matrix_dirty = true;
	m_coordinate_system = rot;
	m_final_rotation = m_rotation * m_coordinate_system;
	refresh_vectors();
}
...
math::quatf const& Camera::get_final_rotation() const
{
...
void Camera::update_projection_matrix() const
{
	...
	if (m_projection_matrix_dirty || ar != m_aspect_ratio_cached)
	{
		...
		if (m_is_parallel)
		{
			float y_half_size = m_zoom * 0.5f;
			float x_half_size = m_zoom * ar * 0.5f;

			m_projection_matrix = math::ortho(-x_half_size, x_half_size, -y_half_size, y_half_size, m_near_distance, m_far_distance);
		}
		else
		{
...
void Camera::update_view_matrix() const
{
	...
	auto const& rot = get_final_rotation();

	// the View matrix is created by taking the world rotation and translation and then computing the inverse of that matrix.
	// we also have to take into account the axis change from q to opengl coordinate system just before making the inverse.
	math::trans3df tmp_view_transform(math::vec3f::zero, rot, math::vec3f::one);
	tmp_view_transform.set_translation(pos);

	// now we have the view transform in q world coordinates... convert it to opengl world coordinates
