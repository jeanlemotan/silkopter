...
public:
    ...
    uint32_t		addTexture(std::string const& name, video::Texture_cptr const& tex);
    ...
    std::string const&     getTextureName(uint32_t idx) const;

    int             findAnimationIdxByName(std::string const& name) const;
    std::string const&     getAnimationName(uint32_t idx) const;
...
inline uint32_t SpriteBase::addTexture(std::string const& name, video::Texture_cptr const& tex)
{
    QASSERT(tex);
    Texture t;
    t.name = name;
...
inline uint32_t SpriteBase::addAnimation(std::string const& name)
{
    Anim a;
    a.name = name;
    mAnims.push_back(a);
...
inline uint32_t SpriteBase::addFrame(std::string const& name)
{
    Frame f;
    f.name = name;
    mFrames.push_back(f);
...
inline std::string const& SpriteBase::getTextureName(uint32_t idx) const
{
    return mTextures[idx].name;
}


inline std::string const& SpriteBase::getAnimationName(uint32_t idx) const
{
    return mAnims[idx].name;
}
inline int SpriteBase::findAnimationIdxByName(std::string const& name) const
{
