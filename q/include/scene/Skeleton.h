...
public:
    ...
    std::string const&	get_node_name(size_t idx) const;
    int					find_node_idx_by_name(std::string const& name) const;

    int					add_node(std::string const& name, size_t parent_idx);
...
inline std::string const& Skeleton::get_node_name(size_t idx) const
{
    return m_names[idx];
}
