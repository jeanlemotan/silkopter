...
struct Frustum
{
    ...
    Cull_Status cull_bounds_around(util::Bounds const& bounds) const;
...
protected:
    ...
    mutable bool m_dirty_planes = false;
	mutable math::planef m_planes[(int)Plane::COUNT];

    mutable bool m_dirty_points = false;
...
inline Cull_Status Frustum::cull_bounds_around(util::Bounds const& bounds) const
{
	if (m_dirty_planes)
	{
		compute_planes();
