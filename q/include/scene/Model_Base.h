...
namespace scene
{
	...
	{
		DEFINE_RTTI_CLASS(q::scene::Model_Base, q::res::Resource);
	public:
		...
		size_t				add_mesh(String const& name, size_t node_idx, size_t material_idx, video::Vertex_Declaration const& declaration, math::aabb3f const& aabb);
		...
		size_t				add_material(String const& name, video::Material const& material);
		size_t				get_material_count() const;
		Material_Data const&get_material(size_t idx);
		int					find_material_idx_by_name(String const& name);
		...
		size_t				add_texture_channel(String const& name, String const& textureName, video::Sampler::Wrap wrapu, video::Sampler::Wrap wrapv);
