...
public:
    ...
    struct Attribute
    {
        std::string		name; //for user semantics this contains the name of the attribute. Otherwise it's empty
...
public:
    ...
    auto add_attribute(Vertex_Buffer_ptr buffer, std::string const& name, Type type, uint8_t count, uint16_t offset, uint16_t stride) -> int;

    auto find_attribute_idx_by_name(std::string const& name) const -> int;
...
private:
    auto add_attribute(Vertex_Buffer_ptr buffer, std::string const& name, Semantic semantic, Type type, uint8_t count, uint16_t offset, uint16_t stride) -> int;
