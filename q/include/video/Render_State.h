...
public:
    struct Blend_Formula
    {
        ...
        Factor src_color = Factor::ONE;
        Factor src_alpha = Factor::ONE;
        Factor dst_color = Factor::ZERO;
        Factor dst_alpha = Factor::ZERO;
...
protected:
    ...
    Blend_Formula::Factor	parse_blend_factor(std::string const& str, Blend_Formula::Factor def);
    Compare_Func			parse_compare_func(std::string const& str, Compare_Func def);
    Stencil_Op				parse_stencil_op(std::string const& str, Stencil_Op def);

    Flags					m_flags = Flags(Flag::CULL, Flag::DEPTH_TEST, Flag::DEPTH_WRITE);

    Blend_Formula			m_blend_formula = Blend_Formula(Blend_Formula::Preset::SOLID);

    Cull_Face				m_cull_face = Cull_Face::BACK;
    Winding					m_winding = Winding::WINDING_CCW;

    Compare_Func			m_depth_func = Compare_Func::LEQUAL;

    // Stencil
    Compare_Func			m_stencil_func = Compare_Func::LEQUAL;
    int32_t					m_stencil_ref = 1;
    uint32_t				m_stencil_mask = ~0u;

    Stencil_Op				m_front_stencil_pass_op = Stencil_Op::KEEP;
    Stencil_Op				m_front_stencil_s_fail_op = Stencil_Op::KEEP;
    Stencil_Op				m_front_stencil_z_fail_op = Stencil_Op::KEEP;

    Stencil_Op				m_back_stencil_pass_op = Stencil_Op::KEEP;
    Stencil_Op				m_back_stencil_s_fail_op = Stencil_Op::KEEP;
    Stencil_Op				m_back_stencil_z_fail_op = Stencil_Op::KEEP;
};
