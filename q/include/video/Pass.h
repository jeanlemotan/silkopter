...
public:
    ...
    auto get_scope_name() const -> std::string const&;
    ...
    auto find_material_uniform_def_idx_by_name(std::string const& name) const -> int;
    ...
    auto find_uniform_def_idx_by_name(std::string const& name) const -> int;
    ...
    auto find_material_sampler_def_idx_by_name(std::string const& name) const -> int;

    auto get_sampler_def_count() const -> size_t;
    auto get_sampler_def(size_t sampler_idx) const -> Sampler_Def const&;
    auto find_sampler_def_idx_by_name(std::string const& name) const -> int;
    ...
    auto find_attribute_def_idx_by_name(std::string const& name) const -> int;
...
private:
    ...
    size_t		m_scope_index = 0; //the renderer holds a mask of the active scopes
    ...
    std::map<std::string, size_t> m_material_uniform_map;

    //samplers
    std::vector<Sampler_Def>	m_samplers;

    std::vector<size_t>	m_material_samplers_idx;
    std::map<std::string, size_t> m_material_sampler_map;

    // attributes
    std::vector<Attribute_Def>	m_attributes;
    std::map<std::string, size_t> m_attribute_map;
