...
namespace res
{
	...
	public:
		...
		template<class T>	auto load_by_name(String const& name) const -> std::shared_ptr<T>;
	...
	private:
		...
		typedef std::map<int, std::unique_ptr<Streamer_Wrapper_Base>> Class_Id_To_Streamer;
	...
	template<class T> void Factory::register_loader(std::unique_ptr<Loader<T>> loader)
	{
		...
		int class_id = rtti::get_class_id<T>();
	...
	template <class T> auto Factory::load(Path const& path) const -> std::shared_ptr<T>
	{
		...
		std::shared_ptr<T> tres = rtti::cast_to<T>(res);
		QASSERT(!res || tres);
		return tres;
	}
	template <class T> auto Factory::load_by_name(String const& name) const -> std::shared_ptr<T>
	{
		std::shared_ptr<Streamer<T>> streamer;
