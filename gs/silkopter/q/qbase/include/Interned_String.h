...
protected:
	...
	{
		Cell() 
			: length(0), hash(0), key(get_null()), string(get_null()), next(nullptr), bext_sibling(nullptr), counter_data(0), counter(&counter_data) 
		{
		}
		Cell(atomic_count* _counter)
			: length(0), hash(0), key(get_null()), string(get_null()), next(nullptr), bext_sibling(nullptr), counter_data(0), counter(_counter) 
		{
		}

		int length; //length for both string and key

		uint32_t hash; //hash for key
		...
		Cell* next; //points to the next family
		Cell* bext_sibling; //points to the next string in the family
