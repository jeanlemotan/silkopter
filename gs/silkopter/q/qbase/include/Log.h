...
//	A game will game a defautl topic and several user topics like 'online', 'multiplayer', 'engine', 'resources' etc
	...
	void set_topic_enabled(String const& topic, bool enabled);
...
//	All decorations appear in square paranthesis []. The order is of the decorations is the one in the enum: LEVEL, TOPIC, LOCATION, DATE, TIME
	...
	void set_decorations(Decorations decorations);
	void set_decorations(String const& topic, Decorations decorations);
...
//	The log level used is defined as max(defaultLevel, topicLevel)
	...
	void set_level(String const& topic, Level level);
...
// 		 Most of the time the topic is a char const* 
	...
	void log(logging::Level level, const String& topic, const char* file, int line, const String& message);

	template<typename... Params>
	void logf(logging::Level level, String const& topic, char const* file, int line, String const& fmt, Params... params)
	{
		String message;
		q::util::format(message, fmt, params...);
