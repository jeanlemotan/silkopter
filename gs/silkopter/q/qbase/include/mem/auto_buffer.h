...
namespace boost
{
    ...
    private:
        ...
        pointer move_to_new_buffer( size_type new_capacity )
        {
        ...
        void reserve_impl( size_type new_capacity )
        {
            ...
            QASSERT( size_ <= members_.capacity_ );
        }

        size_type new_capacity_impl( size_type n )
        {
            QASSERT( n > members_.capacity_ );
        ...
        {
            QASSERT( l.is_on_stack() && r.is_on_stack() );
        ...
        {
            QASSERT( size_ + n <= members_.capacity_ );
            size_ += n;
        }
        ...
        {
            QASSERT( size_ + 1 <= members_.capacity_ );
            size_ += 1;
        }
    ...
    public:
        ...
        auto_buffer( size_type size_arg, optimized_const_reference init_value,
            ...
            : allocator_type( a ),
              ...
              size_( 0 )
        {
            std::uninitialized_fill( buffer_, buffer_ + size_, init_value );
            size_ = size_arg;
            QASSERT( is_valid() );
        }
    ...
    public:
        ...
        {
            QASSERT( size_ + std::distance(begin_arg, end_arg) <= members_.capacity_ );
        ...
        {
            QASSERT( !(std::distance(from,to)>0) ||
                          !empty() );
            QASSERT( cbegin() <= from );
            QASSERT( cend() >= to );
        ...
        void shrink_to_fit()
        {
            ...
            QASSERT( is_on_stack() || size_ == members_.capacity_ );
            QASSERT( !is_on_stack() || size_ <= members_.capacity_ );
        }
        ...
        void swap( auto_buffer& r )
        {
            ...
            if( exactly_one_on_stack )
            {
                ...
                QASSERT( other->is_on_stack() );
                QASSERT( !one_on_stack->is_on_stack() );
                QASSERT( is_valid() );
                QASSERT( r.is_valid() );
                return;
            }
