...
void Zip_Data_Pack::load()
{
	...
	while (s.get_offset() < s.get_size())
	{
		...
		//PASSERT2(zipData.header.dataDescriptor.compressedSize == zipData.header.dataDescriptor.uncompressedSize, "Zip '{}' file '{}' is compressed. Only uncompressed files are supported.", m_path, strbuffer);
		...
		//QLOG_INFO("Q", "FILE {}:{}", m_files.size(), str_buffer);
...
// 	PINFO("Loaded file system '{}' ({} files, {} directories): {}s, {}Kb", m_path, fileCount, dirCount, duration, x / 1024.f);
}
