...
void GLES_Fbo_Wrapper::clear(gles::Interface& interf, math::vec3f const& color, float depth, uint32_t mask)
{
	interf.iglDepthMask(gles::iGL_TRUE);
	interf.iglColorMask(gles::iGL_TRUE, gles::iGL_TRUE, gles::iGL_TRUE, gles::iGL_TRUE);

	interf.iglClearColor(color.x, color.y, color.z, 1.f);
...
void GLES_Fbo_Wrapper::bind(gles::Interface& interf, math::vec3f const& color, float depth)
{
...
void GLES_Fbo_Wrapper::set_color_clear_value(math::vec3f const& color)
{
	m_color_clear_value = color;
}
auto GLES_Fbo_Wrapper::get_color_clear_value() const -> math::vec3f const&
{
