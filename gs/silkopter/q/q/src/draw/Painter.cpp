...
#define MAX_SIN_COS_TABLE_SIZE	2048u
...
void Painter::draw_arc(Vertex const& center, math::anglef start_angle, math::anglef end_angle, float radius, size_t tesselation)
{
	...
	start_angle = math::anglef::_2pi - start_angle;//ccw
	end_angle = math::anglef::_2pi - end_angle;//ccw
	if (start_angle.radians > end_angle.radians)
	{
	...
	if (interval > 0.0f && radius > 0.0f)
	{
		if (tesselation < 3)
		{
			...
			tesselation = math::max((size_t)(tesselation * (interval/(2.0f*pi))), 4u);// tesselation is in perimeter space so here we apply a ratio because of the arc length
		}
...
void Painter::add_lines()
{
	...
	size_t count = math::min(m_batch_infos[m_batch_infos_idx].element_count, 32767u);
	m_batch_infos_idx++;
...
void Painter::add_quads_and_tris()
{
	size_t start_pos = m_bytecode_idx;
	size_t end_pos = start_pos + m_batch_infos[m_batch_infos_idx].byte_count;
	size_t count = math::min(m_batch_infos[m_batch_infos_idx].element_count, 16384u);
	m_batch_infos_idx++;
	m_bytecode_idx = end_pos;
