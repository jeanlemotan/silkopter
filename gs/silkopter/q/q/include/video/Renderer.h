...
namespace video
{
	...
	public:
		...
		struct Hdr_Params
		{
			Hdr_Params() : start_range(0), end_range(1) {}
		...
		virtual void			set_user_uniform_getter(String const& name, Uniform_Def::Getter const& getter) = 0;
		virtual void			set_user_sampler_getter(String const& name, Sampler_Def::Getter const& getter) = 0;

		virtual void			add_render_target(String const& name, Render_Target_ptr rt) = 0;
		virtual Render_Target_ptr	find_render_target_by_name(String const& name) = 0;
