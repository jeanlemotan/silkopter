...
namespace video
{
	...
	protected:
		...
		{
		public:
			Render_Context() : command_stream(nullptr), pass_idx(0) {}
	...
	private:
		...
		struct Post_FX_Data
		{
			...
			bool			is_rendering; //true when this pfx is being rendered into
			bool			is_flushing; //true when this pfx is being rendered on screen
		...
		bool	m_needs_full_clear_every_frame;

		struct Hdr
		{
			Hdr() : format(Hdr_Format::NONE) {}
		...
		bool				m_default_render_target_binded;

		Command_Stream*		m_command_streams[2];

		Capabilities		m_capabilities;
		bool				m_capabilities_initialized;
