...
namespace video
{
	...
	protected:
		...
		uint32_t	compile_shader(uint32_t shader_type, String const& source);

		void		upload_data(gles::Interface& interf, const Command_Stream::Render_Context& context) const;

		String		patch_shader(std::vector<Uniform_Def>& uniforms,
								std::vector<Sampler_Def>& samplers,
								std::vector<Attribute_Def>& attributes,
	...
	private:
		...
		String		m_patched_vertex_shader_src;
		String		m_patched_fragment_shader_src;
		...
		size_t			m_max_skin_nodes;

		size_t			m_max_uniforms;
		size_t			m_max_samplers;
		size_t			m_max_attributes;
