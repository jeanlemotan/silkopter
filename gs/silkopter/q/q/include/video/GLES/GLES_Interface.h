...
namespace gles
{
	...
	static const uint32_t iGL_DEPTH24_STENCIL8_EXT           = 0x88F0;
...
#		endif
	...
	public:
		...
		//when an object is changed, this forces the shadow state to dirty for that object in all Thread Interfaces
		void force_array_buffer_rebinding(iGLuint buffer);
		void force_element_buffer_rebinding(iGLuint buffer);
		void force_framebuffer_rebinding(iGLuint buffer);
		void force_renderbuffer_rebinding(iGLuint buffer);
		void force_program_rebinding(iGLuint program);
		void force_texture_rebinding();

		//when an object is deleted, these functions remove the reference from all the Thread Interfaces
		static void remove_buffer(uint32_t buffer);
		static void remove_framebuffer(uint32_t buffer);
		static void remove_program(uint32_t program);
		static void remove_renderbuffer(uint32_t buffer);
		static void remove_texture(uint32_t texture);
