...
namespace video
{
	...
	public:
		...
		void		copy_from(int const* val, size_t count);
	...
	inline Uniform::Uniform(int val)
		: m_type(Type::INT)
		, m_count(0)
		, m_data(nullptr)
	{
	...
	inline Uniform& Uniform::operator=(int val)
	{
		set(0, val);
		return *this;
	}
	...
	inline size_t Uniform::get_data_size() const
	{
		...
		QASSERT((int)m_type < sizeof(size) / sizeof(size_t));
		return size[(int)m_type] * m_count;
	}
	...
	inline int Uniform::get_as_int32(size_t idx) const
	{
		QASSERT(get_type() == Type::INT);
		QASSERT(idx < get_count());
		return ((int*)m_data)[idx];
	}
	...
	inline void Uniform::set(size_t idx, int val)
	{
	...
	inline void Uniform::copy_from(const int* val, size_t count)
	{
		QASSERT(get_type() == Type::INT && count <= get_count());
		memcpy(m_data, val, count * sizeof(int));
	}
