...
public:
	...
	struct Attribute
	{
		Attribute() : semantic(Semantic::USER), type(Type::FLOAT), count(0), buffer_idx(0), offset(0), stride(0) {}
		String		name; //for user semantics this contains the name of the attribute. Otherwise it's empty
		Semantic	semantic;
		Type		type; //component type
		uint8_t		count; //how many components per vector
		uint8_t		buffer_idx; //the index of the vertex buffer
...
private:
	...
	{
		Attribute_Data() : data_size(0), write_address(0), read_address(0) {}

		Attribute	attribute;
		size_t		data_size;
		mutable uint8_t*			write_address;
		mutable uint8_t const*	read_address;
	};
