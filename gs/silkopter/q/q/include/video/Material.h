...
//		Material(Material&& other); //CV: until I fix the swap semantics for the EmbeddedAllocator
		...
		int					find_uniform_idx_by_name(size_t pass_idx, String const& name) const;
		size_t				get_uniform_count(size_t pass_idx) const;
		String				get_uniform_name(size_t pass_idx, size_t uniform_idx) const;
		...
		int					find_sampler_idx_by_name(size_t pass_idx, String const& name) const;
		size_t				get_sampler_count(size_t pass_idx) const;
		String				get_sampler_name(size_t pass_idx, size_t sampler_idx) const;
	...
	private:
		...
		struct PassData
		{
			PassData() : uniform_start_idx(0), sampler_start_idx(0), sampler_count(0) {}

			Render_State render_state;
			size_t uniform_start_idx;
			size_t sampler_start_idx;
	...
	inline int Material::find_pass_idx_by_name(String const& name) const
	{
	...
	inline String Material::get_uniform_name(size_t pass_idx, size_t uniform_idx) const
	{
		return m_technique ? m_technique->get_pass(pass_idx).get_material_uniform_def(uniform_idx).get_name() : String::null;
	}

	inline int Material::find_uniform_idx_by_name(size_t pass_idx, String const& name) const
	{
	...
	inline int Material::find_sampler_idx_by_name(size_t pass_idx, String const& name) const
	{
	...
	inline String Material::get_sampler_name(size_t pass_idx, size_t sampler_idx) const
	{
		return m_technique ? m_technique->get_pass(pass_idx).get_material_sampler_def(sampler_idx).get_name() : String::null;
	}
