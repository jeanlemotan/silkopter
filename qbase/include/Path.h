...
public:
    ...
    auto operator[](int idx) const -> std::string;
    ...
    auto get_as_string() const -> std::string;
    ...
    auto get(int idx) const -> std::string const&;
    ...
    auto get_extension_part() const -> std::string;
    ...
    auto get_name_part() const -> std::string;
...
private:
    ...
    static const std::string        k_back;

    void parse(char const* path, size_t size);
    auto can_pop() const -> bool;

    typedef mem::Embedded_Allocator<std::string, 16*sizeof(std::string)> Allocator;
    Allocator::arena_type m_arena;

    std::vector<std::string, Allocator>		m_elements;
...
inline auto Path::operator[](int idx) const -> std::string
{
    return get(idx);
}
...
inline auto Path::get_as_string() const -> std::string
{
...
inline auto Path::get(int idx) const -> std::string const&
{
...
inline auto Path::get_filename() const -> std::string
{
    return empty() ? std::string() : m_elements.back();
}
inline auto Path::get_extension_part() const -> std::string
{
    if (m_elements.empty())
    {
        return std::string();
    ...
    std::string const& fn = m_elements.back();
...
inline auto Path::get_name_part() const -> std::string
{
    if (m_elements.empty())
    {
        return std::string();
...
inline auto Path::pop_front() -> std::string
{
    if (m_elements.empty())
    {
        return std::string();
...
inline auto Path::pop_back() -> std::string
{
    if (m_elements.empty())
    {
        return std::string();
