...
public:
    ...
    auto init(node::IUART* bus, Init_Params const& params) -> bool;

    void process();

    auto get_name() const -> q::String const&;
    auto get_samples() const -> std::vector<node::IGPS::Sample> const&;
...
private:
    ...
    struct Sample
    {
        ...
        bool has_sol = false;
        node::IGPS::Location data;
        std::vector<node::IGPS::Sample> samples;
