...
UBLOX::UBLOX(q::String const& name)
    : m_name(name)
{
}
...
auto UBLOX::init(node::II2C* bus, Init_Params const& params) -> bool
{
    m_i2c = bus;
    m_spi = nullptr;
    m_uart = nullptr;
...
auto UBLOX::init(node::ISPI* bus, Init_Params const& params) -> bool
{
    m_spi = bus;
    m_i2c = nullptr;
    m_uart = nullptr;
...
auto UBLOX::init(node::IUART* bus, Init_Params const& params) -> bool
{
    m_spi = nullptr;
    m_i2c = nullptr;
    m_uart = bus;
...
void UBLOX::process()
{
    ...
    if (m_sample.has_nav_status && m_sample.has_pollh && m_sample.has_sol)
    {
        ...
        m_sample.data = node::IGPS::Location();
    }
...
//    0x05 = Time only fix
...

    {
        ...
        if (data.gpsFix == 0x02)
        {
            m_sample.data.fix = node::IGPS::Location::Fix::FIX_2D;
        }
        else if (data.gpsFix == 0x03)
        {
            m_sample.data.fix = node::IGPS::Location::Fix::FIX_3D;
        }
...
auto UBLOX::get_samples() const -> std::vector<node::IGPS::Sample> const&
{
