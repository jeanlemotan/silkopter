...
//    0x05 = Time only fix
...

    {
        ...
        if (data.gpsFix == 0x02)
        {
            m_sample.data.fix = sensor::Location::Fix::FIX_2D;
        }
        else if (data.gpsFix == 0x03)
        {
            m_sample.data.fix = sensor::Location::Fix::FIX_3D;
        }
...
auto UBLOX::get_samples() const -> std::vector<sensor::GPS_Sample> const&
{
