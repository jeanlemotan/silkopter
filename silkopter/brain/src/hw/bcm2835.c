...
uint8_t bcm2835_i2c_write(const char * buf, uint32_t len)
{
    ...
    int st = bcm2835_peri_read_nb(status);
    while(reason == BCM2835_I2C_REASON_OK &&
          (st & BCM2835_BSC_S_DONE) == 0)
    {
        while (reason == BCM2835_I2C_REASON_OK &&
                remaining &&
                (st & BCM2835_BSC_S_TXD) != 0)
        {
            /* Write to FIFO, no barrier */
            bcm2835_peri_write_nb(fifo, buf[i]);
            i++;
            ...
            st = bcm2835_peri_read_nb(status);
            CHECK_ERROR;
        }

        if (timeout_checker-- < 0 && bcm2835_st_read() > start + TIMEOUT_US)
        {
    ...
    if (reason == BCM2835_I2C_REASON_OK && remaining)
    {
...
{   
    ...
    int st = bcm2835_peri_read(status);
    while (reason == BCM2835_I2C_REASON_OK &&
           (st & (BCM2835_BSC_S_TA | BCM2835_BSC_S_DONE)) == 0)
    {
        if (timeout_checker-- < 0 && bcm2835_st_read() > start + TIMEOUT_US)
        {
            timeout_checker = TIMEOUT_CHECKER;
