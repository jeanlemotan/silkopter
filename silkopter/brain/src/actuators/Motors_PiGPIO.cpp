...
auto Motors_PiGPIO::load_settings() -> bool
{
...
void Motors_PiGPIO::save_settings()
{
}

auto Motors_PiGPIO::init() -> bool
{
...
auto Motors_PiGPIO::get_pulse_range() const -> std::pair<int, int>
{
    if (m_settings.frequency < PWM_Frequency::PWM_1000HZ)
    {
...
void Motors_PiGPIO::cut_throttle()
{
    QASSERT(m_is_initialized);
    if (!m_is_initialized)
    {
...
void Motors_PiGPIO::set_throttles(float const* throttles, size_t count)
{
    QASSERT(m_is_initialized);
    if (!m_is_initialized)
    {
