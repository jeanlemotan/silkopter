...
ts::Result<void> deserialize(IUAV_Descriptor::Comms& value, ts::sz::Value const& sz_value)
{
  if (!sz_value.is_object()) { return ts::Error("Expected object value when deserializing"); }
  {
    auto const* member_sz_value = sz_value.find_object_member_by_name("rc_frequency");
    if (!member_sz_value) { return ts::Error("Cannot find member value 'rc_frequency'"); }
    std::remove_cv<std::remove_reference<decltype(value.get_rc_frequency())>::type>::type v;
    auto result = deserialize(v, *member_sz_value);
    if (result != ts::success) { return result; }
    value.set_rc_frequency(std::move(v));
  }
  {
...
ts::sz::Value serialize(IUAV_Descriptor::Comms const& value)
{
  ...
  sz_value.add_object_member("rc_frequency", serialize(value.get_rc_frequency()));
  sz_value.add_object_member("rc_xtal_ajdustment", serialize(value.get_rc_xtal_ajdustment()));
  ...
  sz_value.add_object_member("video_wlan_interface", serialize(value.get_video_wlan_interface()));
  return sz_value;
}
