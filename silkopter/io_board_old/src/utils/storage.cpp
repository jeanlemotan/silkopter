...
// 			}
	...
	void init()
	{
        ...
        q::quick_logf(F_STR("Storage: crc {}, count {}"), s_main_header.crc, s_main_header.count);

		auto crc = _compute_main_crc();
		if (crc != s_main_header.crc)
		{
            q::quick_logf(F_STR("Storage corrupted. Formatting..."));
			remove_all();
		}
 		else
 		{
            q::quick_logf(F_STR("CRC is ok"));
 		}
	...
	void set_record(Id id, uint8_t const* data, size_type size)
	{
		...
		if (offset != 0)
		{
            ...
            auto crc = q::util::compute_crc<q::util::crc16_t>(data, size);
		...
		else
		{
			...
			header.id = id;
			header.size = size;
            header.crc = q::util::compute_crc<q::util::crc16_t>(data, size);
