...
public:
    ...
    void swap();
    void display();
    bool displayIncremental(uint32_t micros);
    ...
    void drawPixelWhite(int16_t x, int16_t y);
    void drawPixelBlack(int16_t x, int16_t y);

private:
    uint8_t* m_draw_buffer = nullptr; // Pointer to OLED data buffer in memory

    uint8_t* m_show_buffer = nullptr; // Pointer to OLED data buffer in memory
    int16_t m_show_buffer_offset = 0;

    std::array<std::vector<uint8_t>, 2> m_buffers;
    int16_t m_buffer_size;

    int8_t m_i2c_addr, m_dc, m_rst, m_cs;
    int16_t m_width, m_height;
    uint8_t m_vcc_type;
    uint8_t m_oled_type;
    uint8_t m_gray_h, m_gray_l;
