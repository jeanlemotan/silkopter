...
inline void ArduiPi_OLED::fastI2Cwrite(char* tbuf, uint32_t len) {
    ...
    m_dev.write(m_i2c_addr, reinterpret_cast<uint8_t const*>(tbuf), len);
}
...
void ArduiPi_OLED::drawPixelWhite(int16_t x, int16_t y)
{
    ...
    if (m_oled_type == OLED_SEEED_I2C_96x96 )
    {
        // Get where to do the change in the buffer
        uint8_t* p = m_draw_buffer + (x + (y/2)*m_width );
    ...
    {
        // Get where to do the change in the buffer
        uint8_t* p = m_draw_buffer + (x + (y >> 3)*m_width );
        *p |=  _BV((y&7));
    }
}
void ArduiPi_OLED::drawPixelBlack(int16_t x, int16_t y)
{
    if ((x < 0) || (x >= m_width) || (y < 0) || (y >= m_height))
    {
        return;
...
bool ArduiPi_OLED::select_oled(uint8_t OLED_TYPE)
{
    ...
    case OLED_ADAFRUIT_I2C_128x32:
        m_height = 32;
        m_i2c_addr = ADAFRUIT_I2C_ADDRESS;
        break;
    ...
    case OLED_SEEED_I2C_128x64:
        m_i2c_addr = SEEED_I2C_ADDRESS ;
        m_vcc_type = SSD_External_Vcc;
        break;
    ...
    if ( OLED_TYPE == OLED_SEEED_I2C_96x96 )
        m_buffer_size = m_buffer_size / 2 ;
    else
        m_buffer_size = m_buffer_size / 8;

    for (std::vector<uint8_t>& buffer: m_buffers)
    {
        buffer.resize(m_buffer_size);
    }

    // Allocate memory for OLED buffer
    m_draw_buffer = m_buffers[0].data();
...
bool ArduiPi_OLED::init(uint8_t OLED_TYPE)
{
...
void ArduiPi_OLED::begin( void ) 
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (m_oled_type == OLED_SH1106_I2C_128x64)
                contrast = 0x80;
            else
                contrast = (m_vcc_type==SSD_External_Vcc?0x9F:0xCF);
        }

    }

    if (m_vcc_type == SSD_External_Vcc)
    {
        chargepump = 0x10;
        precharge  = 0x22;
    }
    ...
    else if (m_oled_type == OLED_SH1106_I2C_128x64)
    {
...
{
    m_gray_h = (grayLevel << 4) & 0xF0;
    m_gray_l =  grayLevel & 0x0F;
}
...
void ArduiPi_OLED::putSeedChar(char C)
{
    ...
    for(uint8_t i=0;i<8;i=i+2)
    {
        for(uint8_t j=0;j<8;j++)
        {
            ...
            c|=(bit1)?m_gray_h:0x00;
            c|=(bit2)?m_gray_l:0x00;
            sendData(c);
        }
    }
}
...
void ArduiPi_OLED::invertDisplay(uint8_t i) 
{
    if (i)
        ...
        sendCommand(m_oled_type==OLED_SEEED_I2C_96x96 ? SSD1327_Normal_Display : SSD1306_Normal_Display);
}
...
void ArduiPi_OLED::swap()
{
    if (m_draw_buffer == m_buffers[0].data())
    {
        m_show_buffer = m_buffers[0].data();
        m_draw_buffer = m_buffers[1].data();
    }
    else
    {
        m_show_buffer = m_buffers[1].data();
        m_draw_buffer = m_buffers[0].data();
    }

    memcpy(m_draw_buffer, m_show_buffer, m_buffer_size);
}
...
bool ArduiPi_OLED::displayIncremental(uint32_t micros)
{

    if (m_show_buffer_offset == 0)
    {
        swap();
    ...
    // pointer to OLED data buffer
    uint8_t * p = m_show_buffer + m_show_buffer_offset;
    ...
    //  else
    {
        ...
        else
        {
            // loop trough all OLED buffer and
            // send a bunch of 16 data byte in one xmission
            for (uint16_t i = m_show_buffer_offset; i < m_buffer_size; i += 16 )
            {
                memcpy(buff + 1, p, 16);
                p += 16;
                m_show_buffer_offset += 16;

                fastI2Cwrite(buff, 17);

                uint32_t us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start_tp).count();
                if (us >= micros)
                {
                    break;
                }
    ...
    if (m_show_buffer_offset >= m_buffer_size)
    {
        m_show_buffer_offset = 0;
        return true;
    }
...
{
    memset(m_draw_buffer, 0, m_buffer_size);
}
