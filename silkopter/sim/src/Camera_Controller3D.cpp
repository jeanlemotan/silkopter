...
void Camera_Controller3D::mouse_press_event(Mouse_Event const& event)
{
	// update pointer 2d
    m_pointer_2d = event.point;
...
void Camera_Controller3D::mouse_release_event(Mouse_Event const& event)
{
...
void Camera_Controller3D::mouse_move_event(Mouse_Event const& event)
{
    m_pointer_delta_2d = event.point - m_pointer_2d;
    m_pointer_2d = event.point;
...
// 	m_camera.unproject(out, math::vec3f(m_pointer_2d, m_screen_depth));
    ...
    if (m_pointer_pressed && event.buttons.test(Mouse_Event::Button::RIGHT))
	{
...
void Camera_Controller3D::mouse_wheel_event(Mouse_Event const& event)
{
    if (event.buttons.test(Mouse_Event::Button::MIDDLE) || (QApplication::keyboardModifiers() & Qt::AltModifier))
	{
		return;
	}
    ...
    speed = speed * (event.wheel > 0 ? 1.0f : -1.0f);
