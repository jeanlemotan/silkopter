...
void Render_Widget::mousePressEvent(QMouseEvent* event)
{
    ...
    Mouse_Event ev;
    auto pos = event->pos();
    ev.point.set(pos.x(), pos.y());
    ev.buttons.set(Mouse_Event::Button::LEFT, (event->buttons() & Qt::LeftButton) != 0);
    ev.buttons.set(Mouse_Event::Button::MIDDLE, (event->buttons() & Qt::MiddleButton) != 0);
    ev.buttons.set(Mouse_Event::Button::RIGHT, (event->buttons() & Qt::RightButton) != 0);
    mouse_press_signal.execute(ev);
}
...
void Render_Widget::mouseReleaseEvent(QMouseEvent* event)
{
    ...
    ev.buttons.set(Mouse_Event::Button::MIDDLE, (event->buttons() & Qt::MiddleButton) != 0);
    ev.buttons.set(Mouse_Event::Button::RIGHT, (event->buttons() & Qt::RightButton) != 0);
    mouse_release_signal.execute(ev);
}
...
void Render_Widget::wheelEvent(QWheelEvent* event)
{
    ...
    ev.buttons.set(Mouse_Event::Button::MIDDLE, (event->buttons() & Qt::MiddleButton) != 0);
    ev.buttons.set(Mouse_Event::Button::RIGHT, (event->buttons() & Qt::RightButton) != 0);
    ev.wheel = event->delta();
    mouse_wheel_signal.execute(ev);
}
