...
void render_arrow(math::vec3f const& start, math::vec3f const& end, q::video::Material const& material, uint32_t color, q::draw::Painter& painter)
{
	if (!s_is_initialized)
	{
		s_axes_transforms[0] = math::trans3df(math::vec3f::zero, math::quat_axis_y(1.570796f).get_as_mat3(), math::vec3f::one);
		s_axes_transforms[1] = math::trans3df(math::vec3f::zero, math::quat_axis_x(-1.570796f).get_as_mat3(), math::vec3f::one);
...
/*	mat.get_render_state(0).set_depth_test(false);*/
	...
	math::mat3f rot;
	math::multiply(rot, math::look_at(front, up), math::quat_axis_x(-1.570796f).get_as_mat3());
