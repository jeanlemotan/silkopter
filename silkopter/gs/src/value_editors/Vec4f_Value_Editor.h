...
{
public:
    Vec4f_Value_Editor(const Qualified_Value<ts::IVec4f_Value>& qualified_value, std::string const& editor_str, std::string const& suffix_str);
...
private:
    ...
    void		set_value(const ts::vec4f& value);

    Qualified_Value<ts::IVec4f_Value> m_qualified_value;
