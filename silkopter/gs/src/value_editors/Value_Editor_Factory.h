...
public:
    ...
    std::shared_ptr<IValue_Editor>	create_editor(std::shared_ptr<const ts::IValue> value, std::string const& suffix) const;
    std::shared_ptr<IValue_Editor>	create_editor(std::shared_ptr<ts::IValue> value, std::string const& suffix) const;
...
private:
    using Mutable_Creator = std::function<std::shared_ptr<IValue_Editor>(std::shared_ptr<ts::IValue>, std::string const& suffix_str)>;
    using Const_Creator = std::function<std::shared_ptr<IValue_Editor>(std::shared_ptr<const ts::IValue>, std::string const& suffix_str)>;
...
{
    Creators creators;
    creators.mutable_creator = [=](std::shared_ptr<ts::IValue> value, std::string const& suffix_str) -> std::shared_ptr<IValue_Editor>
    {
        std::shared_ptr<Value> v = std::dynamic_pointer_cast<Value>(value);
        if (v)
        {
            return std::make_shared<Editor>(v, suffix_str, fixedConstructorParams...);
        }
        return nullptr;
    };
    creators.const_creator = [=](std::shared_ptr<const ts::IValue> value, std::string const& suffix_str) -> std::shared_ptr<IValue_Editor>
    {
