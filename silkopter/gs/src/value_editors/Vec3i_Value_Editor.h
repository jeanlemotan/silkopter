...
class Vec3i_Value_Editor : public IValue_Editor
{
public:
    Vec3i_Value_Editor(const Qualified_Value<ts::IVec3i_Value>& qualified_value, std::string const& editor_str, std::string const& suffix_str);
...
private:
    ...
    void		set_value(const ts::vec3i& value);

    Qualified_Value<ts::IVec3i_Value> m_qualified_value;
