...
Angular_Velocity_Calibration_Wizard::Angular_Velocity_Calibration_Wizard(silk::HAL& hal, silk::Comms& comms, silk::node::Node_ptr node, size_t output_idx, QWidget* parent)
    : QDialog(parent)
    , m_hal(hal)
    , m_comms(comms)
    , m_node(node)
...
void Angular_Velocity_Calibration_Wizard::prepare_step()
{
    ...
    else if (m_step == Step::COLLECT)
    {
        ...
        m_connection = m_stream->samples_available_signal.connect([this, info, progress](silk::node::stream::Angular_Velocity& stream)
        {
...
void Angular_Velocity_Calibration_Wizard::on_samples_received(silk::node::stream::Angular_Velocity& stream)
{
