#include "ADC_Stream_Viewer_Widget.h"
#include "Comms.h"
...
ADC_Stream_Viewer_Widget::ADC_Stream_Viewer_Widget(QWidget* parent)
{
    setParent(parent);
}

ADC_Stream_Viewer_Widget::~ADC_Stream_Viewer_Widget()
{
...
void ADC_Stream_Viewer_Widget::init(silk::Comms& comms, std::string const& stream_path, uint32_t stream_rate, silk::stream::Type stream_type)
{
    m_comms = &comms;
    ...
    m_connection = m_comms->sig_telemetry_samples_available.connect([this, widget](silk::Comms::ITelemetry_Stream const& _stream)
    {
        ...
        {
            auto const* stream = dynamic_cast<silk::Comms::Telemetry_Stream<silk::stream::IADC> const*>(&_stream);
            if (stream)
            {
