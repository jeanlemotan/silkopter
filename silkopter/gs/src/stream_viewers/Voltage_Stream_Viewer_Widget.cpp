#include "Voltage_Stream_Viewer_Widget.h"
#include "Comms.h"
...
Voltage_Stream_Viewer_Widget::Voltage_Stream_Viewer_Widget(QWidget* parent)
{
    setParent(parent);
}

Voltage_Stream_Viewer_Widget::~Voltage_Stream_Viewer_Widget()
{
...
void Voltage_Stream_Viewer_Widget::init(silk::Comms& comms, std::string const& stream_path, uint32_t stream_rate, silk::stream::Type stream_type)
{
    m_comms = &comms;
    ...
    widget->add_graph("Voltage", "V", Qt::red);
    ...
    m_connection = m_comms->sig_telemetry_samples_available.connect([this, widget](silk::Comms::ITelemetry_Stream const& _stream)
    {
        ...
        {
            auto const* stream = dynamic_cast<silk::Comms::Telemetry_Stream<silk::stream::IVoltage> const*>(&_stream);
            if (stream)
            {
