#include "Pressure_Stream_Viewer_Widget.h"
#include "Comms.h"
...
Pressure_Stream_Viewer_Widget::Pressure_Stream_Viewer_Widget(QWidget* parent)
{
    setParent(parent);
}

Pressure_Stream_Viewer_Widget::~Pressure_Stream_Viewer_Widget()
{
...
void Pressure_Stream_Viewer_Widget::init(silk::Comms& comms, std::string const& stream_path, uint32_t stream_rate, silk::stream::Type stream_type)
{
    m_comms = &comms;
    ...
    widget->add_graph("Pressure", "kPa", Qt::red);
    ...
    m_connection = m_comms->sig_telemetry_samples_available.connect([this, widget](silk::Comms::ITelemetry_Stream const& _stream)
    {
        ...
        {
            auto const* stream = dynamic_cast<silk::Comms::Telemetry_Stream<silk::stream::IPressure> const*>(&_stream);
            if (stream)
            {
