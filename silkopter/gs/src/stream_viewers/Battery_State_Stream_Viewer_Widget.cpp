#include "Battery_State_Stream_Viewer_Widget.h"
#include "Comms.h"
...
Battery_State_Stream_Viewer_Widget::Battery_State_Stream_Viewer_Widget(QWidget* parent)
{
    setParent(parent);
}

Battery_State_Stream_Viewer_Widget::~Battery_State_Stream_Viewer_Widget()
{
...
void Battery_State_Stream_Viewer_Widget::init(silk::Comms& comms, std::string const& stream_path, uint32_t stream_rate, silk::stream::Type stream_type)
{
    m_comms = &comms;
    ...
    widget->add_graph("Charge", "Ah", Qt::red);
    widget->add_graph("Avg C", "A", Qt::green);
    widget->add_graph("Avg V", "V", Qt::blue);
    widget->add_graph("Left", "%", Qt::yellow);
    ...
    m_connection = m_comms->sig_telemetry_samples_available.connect([this, widget](silk::Comms::ITelemetry_Stream const& _stream)
    {
        ...
        {
            auto const* stream = dynamic_cast<silk::Comms::Telemetry_Stream<silk::stream::IBattery_State> const*>(&_stream);
            if (stream)
            {
