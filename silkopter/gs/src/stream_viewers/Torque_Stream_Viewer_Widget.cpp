#include "Torque_Stream_Viewer_Widget.h"
#include "Comms.h"
...
Torque_Stream_Viewer_Widget::Torque_Stream_Viewer_Widget(QWidget* parent)
{
    setParent(parent);
}

Torque_Stream_Viewer_Widget::~Torque_Stream_Viewer_Widget()
{
    auto result = m_comms->set_stream_telemetry_enabled(m_stream_path, false);
    QASSERT(result == ts::success);
}

void Torque_Stream_Viewer_Widget::init(silk::Comms& comms, std::string const& stream_path, uint32_t stream_rate, silk::stream::Type stream_type)
{
    m_comms = &comms;
    ...
    widget->add_graph("x", "Nm", Qt::red);
    widget->add_graph("y", "Nm", Qt::green);
    widget->add_graph("z", "Nm", Qt::blue);
    ...
    m_connection = m_comms->sig_telemetry_samples_available.connect([this, widget](silk::Comms::ITelemetry_Stream const& _stream)
    {
        ...
        {
            auto const* stream = dynamic_cast<silk::Comms::Telemetry_Stream<silk::stream::ITorque> const*>(&_stream);
            if (stream)
            {
