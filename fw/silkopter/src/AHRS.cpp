...
void AHRS::process(board::IMU::Data const& data)
{
	...
	auto const& accel = data.accelerometer.value;
	if (accel.z > 0.1f)
 	{
		...
		float dt = data.gyroscope.dt.count;
		m_euler.x = math::lerp(m_euler.x, accel_pitch_x, dt);
		m_euler.y = math::lerp(m_euler.y, accel_roll_y, dt);
 	}
