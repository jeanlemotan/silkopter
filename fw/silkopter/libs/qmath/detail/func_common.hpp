...
namespace math
{
	...
	template<> inline float sqrt<float, fast>(float const& v) { return ::sqrtf(v); }
	template<> inline float sqrt<float, fast_safe>(float const& v) { return sqrt<float, fast>(is_negative(v) ? 0 : v); }
	...
	template<> inline double sqrt<double, fast_safe>(double const& v) { return sqrt<double, fast>(is_negative(v) ? 0 : v); }
	...
	template<> inline float pow<float, fast_safe>(float const& a, float const& b) { return ::powf(a, b); }

	template<> inline double pow<double, standard>(double const& a, double const& b) { return ::powf(a, b); }
	template<> inline double pow<double, fast>(double const& a, double const& b) { return ::powf(a, b); }
	template<> inline double pow<double, safe>(double const& a, double const& b) { return ::powf(a, b); }
	template<> inline double pow<double, fast_safe>(double const& a, double const& b) { return ::powf(a, b); }
	...
	{
		quat<T> tmp(v);
		tmp.template normalize<T, Policy>();
		return tmp;
	}
